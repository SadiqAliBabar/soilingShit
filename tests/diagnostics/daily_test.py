...
from soiling_analysis.diagnostics.daily import compute_daily_metrics, compute_iam
from soiling_analysis.diagnostics.orientation import _solar_position
from soiling_analysis.diagnostics.soiling import extract_soiling_trend
...
# ---------------------------------------------------------------------------
# Helper for Batch 2 daily tests
# ---------------------------------------------------------------------------

def _make_day_ts(date_str: str, plate: ModuleConfig, n_pts: int = 12) -> pd.DataFrame:
    """Minimal intraday timeseries (midday 11:00-11:55) for compute_daily_metrics."""
    ts = pd.date_range(f"{date_str} 11:00", periods=n_pts, freq="5min")
    poa = np.full(n_pts, 500.0)
    Gn = poa / 1000.0
    Imp_exp = plate.imp_stc * Gn  # dT=0 simplification
    I = Imp_exp * 0.90
    V = np.full(n_pts, plate.vmp_str_stc)
    P = I * V
    return pd.DataFrame({
        "ts":    ts,
        "I":     I,
        ...
        "P":     P,
        "POA":   poa,
        "qflag": np.zeros(n_pts, dtype=np.int64),
    })


# ---------------------------------------------------------------------------
# Test 4 — calendar grid inserts gap rows into daily output (Batch 2)
# ---------------------------------------------------------------------------

def test_4_calendar_grid_adds_gap_rows():
    """compute_daily_metrics with daily_grid_enabled=True fills in gap days.

    Raw timeseries has data only on Jan 1 and Jan 4 (2-day gap Jan 2-3).
    After to_calendar_grid the output must have 4 rows with is_present=False
    on the two missing days.
    """
    plate = ModuleConfig()
    cfg   = PipelineConfig()
    cfg.daily_grid_enabled = True

    raw_df = pd.concat([
        _make_day_ts("2024-01-01", plate),
        _make_day_ts("2024-01-04", plate),
    ], ignore_index=True)

    out = compute_daily_metrics(raw_df, plate, cfg=cfg)

    assert "is_present" in out.columns, "is_present column not added by calendar grid"
    assert len(out) == 4, f"Expected 4 rows (Jan 1-4), got {len(out)}"

    dates_out = [str(d) for d in out["date"].tolist()]
    present   = out["is_present"].tolist()
    assert bool(present[0]) is True,  f"Jan 1 should be present; got {present}"
    assert bool(present[1]) is False, f"Jan 2 (gap) should be absent; got {present}"
    assert bool(present[2]) is False, f"Jan 3 (gap) should be absent; got {present}"
    assert bool(present[3]) is True,  f"Jan 4 should be present; got {present}"

    # NCI metric columns must be NaN for gap rows
    gap_rows = out[~out["is_present"]]
    assert gap_rows["NCI_noon"].isna().all(), "NCI_noon must be NaN for gap rows"


# ---------------------------------------------------------------------------
# Test 5 — calendar grid disabled preserves old gappy-frame output (Batch 2)
# ---------------------------------------------------------------------------

def test_5_calendar_grid_disabled_no_gap_rows():
    """With daily_grid_enabled=False, output has only dates with real data."""
    plate = ModuleConfig()
    cfg   = PipelineConfig()
    cfg.daily_grid_enabled = False
    ...
    assert "is_present" not in out.columns, (
        "is_present must not appear when daily_grid_enabled=False"
    )
    assert len(out) == 2, f"Expected 2 rows (data-only), got {len(out)}"
...
if __name__ == "__main__":
    ...
    tests = [
        ...
        test_4_calendar_grid_adds_gap_rows,
        test_5_calendar_grid_disabled_no_gap_rows,
    ]
    passed = failed = 0
    for fn in tests:
        try:
