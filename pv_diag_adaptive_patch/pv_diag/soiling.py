...
def extract_soiling_trend(daily_df: pd.DataFrame, wash_result: dict,
    ...
    Uses pick_nci_column() to prefer NCI_adaptive_noon > NCI_corrected_noon
    > NCI_noon.  The trimmed least-squares math is unchanged.
    """
    ...
    for (s0, s1) in seg_bounds:
        ...
        if sub[col].notna().sum() < 4:
            segments_out.append(dict(start=s0, end=s1, n_days=len(sub),
                slope_per_day=np.nan, slope_pct_per_day=np.nan, se=np.nan,
        ...
        fit = _trimmed_lr(x, y)
        slope_capped = float(np.clip(fit["slope"], -0.03, 0.01))
        mean_nci = float(np.nanmean(y))
        loss_pct = float(np.clip(1.0 - mean_nci, 0.0, cfg.soiling_loss_cap) * 100.0)
        segments_out.append(dict(start=s0, end=s1, n_days=len(sub),
            slope_per_day=slope_capped, slope_pct_per_day=slope_capped*100,
            se=fit["se"], r2=fit["r2"], soiling_loss_pct=loss_pct,
            mean_nci=mean_nci))
...
def _empty_soiling(reason="no data"):
    return dict(srr_pct_per_day=np.nan, ci_pct_per_day=np.nan,
                ...
                method="none", nci_col_used="none", explainability=reason)
