...
def flatten_document(doc: dict,
                     azimuth_override: float | None = None,
                     tilt_override: float | None = None,
                     rainfall_override: float | None = None,
                     temp_lookup: dict | None = None) -> list[dict]:
    """
    ...
    # Pre-compute timestamp key for temp_lookup matching (FM_OD_PRD uses string timestamps)
    ts_key = ts.strftime("%Y-%m-%d %H:%M:%S") if not pd.isna(ts) else ""
    ...
    for sn in doc.get("sns", []):
        ...
        # Temperature from external lookup (FM_OD_PRD) takes priority over inverter-level temp
        lookup_temp = np.nan
        if temp_lookup and ts_key and inverter_id:
            lookup_temp = temp_lookup.get((ts_key, inverter_id), np.nan)
        ...
        for mppt in sn.get("mppts", []):
            ...
            for pv in mppt.get("pvs", []):
                ...
                # Priority: external lookup (FM_OD_PRD, per inverter) → pv-level field → inverter-level temp
                pv_temperature = (
                    lookup_temp
                    if not pd.isna(lookup_temp)
                    else safe_float(pv.get("pv_temperature", inverter_temp))
                )
...
def load_from_json(path: str,
                   ...
                   tilt_override: float | None = None,
                   rainfall_override: float | None = None,
                   temp_lookup: dict | None = None) -> pd.DataFrame:
...
def load_temp_lookup_from_mongodb(mongo_uri: str,
                                   db_name: str,
                                   collection: str,
                                   dt_from: datetime,
                                   dt_to: datetime) -> dict:
    """
    Loads pv_temperature from FM_OD_PRD (or similar flat inverter-level collection).

    Returns a dict keyed by (normalized_timestamp_str, inverter_sn) → pv_temperature float.
    The timestamp is normalized to "YYYY-MM-DD HH:MM:SS" so it matches both
    the string format in FM_OD_PRD and the parsed timestamps from the main collection.
    """
    try:
    ...
    query = {
        "timestamp": {
            "$gte": dt_from.strftime("%Y-%m-%d %H:%M:%S"),
            "$lte": dt_to.strftime("%Y-%m-%d %H:%M:%S"),
        }
    }

    docs = list(col.find(query, {"sn": 1, "timestamp": 1, "pv_temperature": 1, "_id": 0}))
    print(f"[INFO] Temperature lookup: fetched {len(docs):,} docs from {collection}")

    lookup = {}
    for doc in docs:
        sn = str(doc.get("sn", "")).strip()
        ts_raw = doc.get("timestamp", "")
        try:
            ts_key = pd.to_datetime(ts_raw).strftime("%Y-%m-%d %H:%M:%S")
        except Exception:
            continue
        temp = safe_float(doc.get("pv_temperature", np.nan))
        if sn and not pd.isna(temp) and temp != 0:
            lookup[(ts_key, sn)] = temp

    print(f"[INFO] Temperature lookup: {len(lookup):,} valid (timestamp, inverter) pairs")
    return lookup
...
def main() -> None:
    ...
    ap.add_argument("--temp-collection", default=None,
                    help="MongoDB collection with pv_temperature (e.g. FM_OD_PRD). "
                         "Joined by timestamp + inverter sn and applied to all strings of that inverter.")
    ap.add_argument("--temp-db", default=None,
                    help="MongoDB database for the temperature collection. Defaults to --db if not set.")
    ...
    if args.input:
        ...
        # Build date range here so we can pass it to the temp lookup too
        dt_to = datetime.strptime(args.date_to, "%Y-%m-%d") if args.date_to else datetime.utcnow()
        dt_from = (datetime.strptime(args.date_from, "%Y-%m-%d") if args.date_from
                   else dt_to - timedelta(days=args.days))

        temp_lookup = None
        if args.temp_collection:
            temp_db = args.temp_db or args.db
            print(f"[INFO] Loading temperature from {temp_db}.{args.temp_collection} ...")
            temp_lookup = load_temp_lookup_from_mongodb(
                mongo_uri=args.mongo,
                db_name=temp_db,
                collection=args.temp_collection,
                dt_from=dt_from,
                dt_to=dt_to,
            )
