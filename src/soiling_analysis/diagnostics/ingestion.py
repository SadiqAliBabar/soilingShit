...
def extract_string_meta(string_dfs):
    ...
    for label, g in string_dfs.items():
        ...
        inv_id = str(r["inverter_id"])
        str_id = str(r["string_id"])

        # string_uid: <InverterID>_<StringID> — added by loader.py (Batch 1)
        if "string_uid" in g.columns:
            uid = str(r["string_uid"])
        else:
            uid = f"{_safe_id(inv_id)}_{_safe_id(str_id)}"

        # commissioning date/year — added by loader.py (Batch 1, SCHEMA-DEP C2)
        comm_date = str(r["commissioning_date"]) if "commissioning_date" in g.columns else None
        comm_year = None
        if "commissioning_year" in g.columns and pd.notna(r["commissioning_year"]):
            try:
                comm_year = int(r["commissioning_year"])
            except (ValueError, TypeError):
                pass
        elif comm_date and len(comm_date) >= 4:
            try:
                comm_year = int(comm_date[:4])
            except (ValueError, TypeError):
                pass

        # pv_capacity — present in string rows via loader.py rename
        pv_cap = None
        if "pv_capacity" in g.columns:
            cap_vals = pd.to_numeric(g["pv_capacity"], errors="coerce").dropna()
            if len(cap_vals):
                pv_cap = float(cap_vals.iloc[0])

        # bifacial flag — added by loader.py (Batch 1, SCHEMA-DEP C3)
        bifacial = False
        if "bifacial" in g.columns:
            try:
                bifacial = bool(r["bifacial"])
            except (ValueError, TypeError):
                pass
