...
def fit_single_diode(df: pd.DataFrame, plate: ModuleConfig,
    ...
    # ------------------------------------------------------------------
    # 6.  Solve (Batch 6: robust loss down-weights outliers)
    # ------------------------------------------------------------------
    # "soft_l1" (default) is robust to high-leverage outliers.
    # Set cfg.robust_sdm_loss="linear" to restore the pre-Batch-6 behaviour.
    _loss    = getattr(cfg, "robust_sdm_loss",    "soft_l1") if cfg is not None else "soft_l1"
    _f_scale = getattr(cfg, "robust_sdm_f_scale", 1.0)       if cfg is not None else 1.0
    try:
