"""Plate-parameter inference — DEPRECATED (Batch 8).

infer_plate_params() is no longer called from the pipeline spine.  Manufacturer
specs read by specs.py / loader.py (Batch 1) are used directly as the nameplate.
The per-string n_modules-from-pv_capacity scaling lives in pipeline._get_string_plate()
and is unchanged.

This module is kept in the tree to avoid breaking any external callers but is
no longer part of the production flow.  Do not add new callers.  It will be
removed in a follow-up cleanup once all tests are green without it.

CRITICAL (original notes): plate values represent NEW-MODULE NAMEPLATE at STC.  We do NOT
