...
def _sheet_transients(wb, results, cfg):
    ws = wb.create_sheet("17_Transient_Events")
...
def _sheet_per_string(wb, results, cfg):
    ws = wb.create_sheet("18_Per_String_Detail")
...
def _sheet_cleaning_economics(wb, results, cfg):
    """13_Cleaning_Economics — per-string payback, recommended date, recoverable losses."""
    ws = wb.create_sheet("13_Cleaning_Economics")
    cur = cfg.site.currency
    wash_cost_label = (
        f"{cfg.wash_cost_per_string_pkr} {cur}/string"
        if cfg.wash_cost_per_string_pkr is not None
        else (f"{cfg.wash_cost_per_kw_pkr} {cur}/kWp" if cfg.wash_cost_per_kw_pkr is not None
              else "not configured (defaulted)")
    )
    _banner(ws, "Cleaning Economics",
            f"Tariff={cfg.site.tariff:.1f} {cur}/kWh | Wash cost: {wash_cost_label}", 10)
    rows = []
    for label, r in results["per_string"].items():
        ec = r.get("cleaning_economics") or {}
        sf = r.get("soiling_full") or {}
        rows.append({
            "string_label":              label,
            "srr_pct_per_day":           _scalar(sf.get("srr_pct_per_day")),
            "daily_loss_pkr":            _scalar(ec.get("daily_loss_pkr")),
            "payback_days":              _scalar(ec.get("payback_days")),
            "recommended_cleaning_date": _scalar(ec.get("recommended_cleaning_date")),
            f"recoverable_{cur}":        _scalar(ec.get("recoverable_pkr")),
            "recoverable_kWh":           _scalar(ec.get("recoverable_kwh")),
            "recommendation":            _scalar(ec.get("recommendation")),
            "economics_inputs":          _scalar(ec.get("economics_inputs")),
        })
    _table(ws, 4, pd.DataFrame(rows))
    _autosize(ws, max_w=50)


def _sheet_operational_view(wb, results, cfg):
    """14_Operational_View — last 7-day and 30-day summaries per string."""
    ws = wb.create_sheet("14_Operational_View")
    _banner(ws, "Operational View",
            "Short-horizon NCI, soiling rate, availability for each window", 12)
    windows = getattr(cfg, "operational_view_windows", (7, 30))
    rows = []
    for label, r in results["per_string"].items():
        daily_df = r.get("daily_df")
        sf = r.get("soiling_full") or {}
        c = r.get("classification") or {}
        row: dict = {"string_label": label, "verdict": c.get("verdict", "")}
        if daily_df is not None and not (hasattr(daily_df, "empty") and daily_df.empty):
            try:
                df_d = daily_df.copy()
                df_d["date"] = pd.to_datetime(df_d["date"])
                from .utils import pick_nci_column as _pick_nci
                max_date = df_d["date"].max()
                nci_col = _pick_nci(df_d)
                for w in windows:
                    cutoff = max_date - pd.Timedelta(days=w)
                    sub = df_d[df_d["date"] >= cutoff]
                    nci_vals = pd.to_numeric(sub[nci_col], errors="coerce").dropna()
                    avail = float(len(nci_vals)) / max(w, 1)
                    row[f"mean_nci_{w}d"] = float(nci_vals.mean()) if len(nci_vals) > 0 else np.nan
                    row[f"avail_{w}d"] = avail
                    row[f"n_days_{w}d"] = int(len(nci_vals))
                    # Soiling rate in window
                    if len(nci_vals) >= 4:
                        from scipy.stats import theilslopes
                        try:
                            xw = (sub["date"][nci_vals.index] - sub["date"][nci_vals.index].min()
                                  ).dt.days.values.astype(float)
                            yw = nci_vals.values
                            ts = theilslopes(yw, xw)
                            row[f"srr_pct_per_day_{w}d"] = float(ts.slope) * 100.0
                        except Exception:
                            row[f"srr_pct_per_day_{w}d"] = np.nan
                    else:
                        row[f"srr_pct_per_day_{w}d"] = np.nan
            except Exception:
                for w in windows:
                    row[f"mean_nci_{w}d"] = np.nan
                    row[f"avail_{w}d"] = np.nan
                    row[f"n_days_{w}d"] = 0
                    row[f"srr_pct_per_day_{w}d"] = np.nan
        else:
            for w in windows:
...
def _sheet_inverter_mppt_rollup(wb, results, cfg):
    """15_Inverter_MPPT_Rollup — per-MPPT and per-inverter aggregations."""
    ws = wb.create_sheet("15_Inverter_MPPT_Rollup")
    _banner(ws, "Inverter & MPPT Roll-up",
            "Mean NCI, soiling rate, curtailment %, verdict counts by inverter/MPPT", 12)

    # Collect per-string data into a flat frame
    rows = []
    for label, r in results["per_string"].items():
        m = r.get("meta") or {}
        c = r.get("classification") or {}
        ...
        sf = r.get("soiling_full") or {}
        dq = r.get("data_quality") or {}
        nci_val = a.get("mean_nci_current")
        rows.append({
            "string_label":       label,
            "inverter_id":        m.get("inverter_id", ""),
            "mppt_id":            m.get("mppt_id", ""),
            "verdict":            c.get("verdict", ""),
            "mean_nci":           nci_val,
            "srr_pct_per_day":    sf.get("srr_pct_per_day"),
            "curt_pct":           dq.get("curt_pct"),
            "string_underperform": bool((r.get("curtailment_summary") or {}).get(
                                   "n_string_underperform", 0) > 0),
        })
    if not rows:
        ws.cell(row=4, column=1, value="(no data)").font = _FT_BODY
        return
    flat = pd.DataFrame(rows)

    def _rollup(grp_col, label_col):
        out = []
        for key, sub in flat.groupby(grp_col):
            srr_vals = pd.to_numeric(sub["srr_pct_per_day"], errors="coerce").dropna()
            nci_vals = pd.to_numeric(sub["mean_nci"], errors="coerce").dropna()
            curt_vals = pd.to_numeric(sub["curt_pct"], errors="coerce").dropna()
            verdict_counts = sub["verdict"].value_counts().to_dict()
            out.append({
                label_col:              key,
                "n_strings":            len(sub),
                "mean_nci_median":      float(nci_vals.median()) if len(nci_vals) > 0 else np.nan,
                "mean_nci_mean":        float(nci_vals.mean())   if len(nci_vals) > 0 else np.nan,
                "srr_median_%/day":     float(srr_vals.median()) if len(srr_vals) > 0 else np.nan,
                "curt_pct_mean":        float(curt_vals.mean())  if len(curt_vals) > 0 else np.nan,
                "n_underperform":       int(sub.get("string_underperform", pd.Series()).sum()),
                "verdicts":             str(verdict_counts)[:200],
            })
        return pd.DataFrame(out)

    r = 4
    r = _section(ws, r, "▸ Per-inverter", 12)
    r = _table(ws, r, _rollup("inverter_id", "inverter_id")) + 1
    r = _section(ws, r, "▸ Per-MPPT (inverter × MPPT)", 12)
    flat["inv_mppt"] = flat["inverter_id"].astype(str) + "_" + flat["mppt_id"].astype(str)
    r = _table(ws, r, _rollup("inv_mppt", "inverter_mppt"))
    _autosize(ws, max_w=40)
...
def export_results_to_excel(results, out_path, source_file="", verbose=True):
    ...
    # Batch 9: new sheets 13–15; existing losses/transients/per-string renumbered 16–18
    _sheet_cleaning_economics(wb, results, cfg)
    _sheet_operational_view(wb, results, cfg)
    _sheet_inverter_mppt_rollup(wb, results, cfg)
