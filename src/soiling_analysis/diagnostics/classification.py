...
def classify_string(
    ...
    age_baseline: float = 1.0,  # string's degradation-corrected factor (Batch 4)
) -> dict:
    ...
    # Age-relative bands: scale cuts when the active column is nameplate-referenced.
    # NCI_adaptive_noon and NCI_corrected_noon are already age-normalised — do NOT
    # scale those, it would double-count the degradation correction.
    _nameplate_cols = {"NCI_noon", "NCI_relative_noon"}
    _bands_age_scaled = cfg.age_relative_bands_enabled and col in _nameplate_cols
    _bl = age_baseline if _bands_age_scaled else 1.0
    _band_clean = _BAND_CLEAN * _bl
    _band_lt    = _BAND_LT    * _bl
    _band_mod   = _BAND_MOD   * _bl
    ...
    axes = dict(soiling_band=band, mean_nci_current=mean_nci,
                ...
                bands_age_scaled=_bands_age_scaled)
