...
def decide_sufficiency(
    ...
    drought_flag (Batch 7): when True and data quality would otherwise be
    "Good", the verdict is demoted to "Limited" to signal that the adaptive
    baseline is operating in monsoon/smog fallback mode and results should
    be interpreted with reduced confidence.  The drought override never
    promotes "Poor" — it only demotes "Good".
    """
    ...
    if (dq["avail_pct"] >= cfg.suff_good_avail_pct
        ...
        reason = f"availability {dq['avail_pct']:.0f}%, curt {dq['curt_pct']:.0f}%"
        if drought_flag:
            # Demote Good → Limited during a plant-scope monsoon/smog drought.
            return "Limited", reason + "; drought_flag=True (monsoon/smog fallback active)"
        return "Good", reason
